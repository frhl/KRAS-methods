...
kegg_bool <- grepl('KEGG', msigdb_c2_table$Set.name)
kegg <- msigdb_c2_table[kegg_bool,]
...
for (dbname in names(dblist)){
  ...
  enrichment <- lapply(names(data), function(cur_sheet){
    ...
    if (any(cur_data$significant)){
      ...
      outfile = paste0('derived/KRAS/tables/211001/',cur_sheet,'_',dbname,'_enrichment.csv')
