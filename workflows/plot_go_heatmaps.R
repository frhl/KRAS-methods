...
# prepare files
files <- list.files('derived/KRAS/tables/211001/', pattern = '.csv', full.names = TRUE)
...
# setup out dirs
out_dir <- 'derived/KRAS/plots/211010_GO_heatmaps'
dir.create(out_dir)
...
for (group in groups){
  ...
  lapply(analyses, function(analysis){
    ...
    dts_ordered$significant <- dts_ordered$FDR < 0.01
    ...
    analysis_subtitle = '(*) indicates significant at FDR < 0.01'
      
    # setup plotting
    plt <- ggplot(dts_subsetted, aes(x = name_minimal, y = list_name, fill = FDR, label = label)) +
      geom_tile() +
      geom_text() +
